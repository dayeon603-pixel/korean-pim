...
> ⚠ **먼저 [DATA_NOTICE.md](DATA_NOTICE.md)를 읽으십시오.**
> 데이터는 2차 요약 경로로 디지털화됐고 **원문 대조가 완료되지 않았습니다.**
> 현재 버전을 임상 의사결정에 그대로 쓰지 마십시오. 연구·프로토타입·교육용입니다.
...
```bash
npm test   # 63건: 데이터 무결성 · 표1 전수 · 표2 18조건 · 거짓양성 검증
```
